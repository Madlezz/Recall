  ...
  return (
    <AlertDialogPrimitive.Portal>
      ...
      <AlertDialogPrimitive.Content
        className={cn(
          "fixed left-1/2 top-1/2 z-50 w-[min(92vw,480px)] -translate-x-1/2 -translate-y-1/2 border bg-card p-6 text-foreground shadow-sm",
          className,
        )}
        {...props}
      />
...
export function AlertDialogTitle({
  ...
  return <AlertDialogPrimitive.Title className={cn("text-base font-medium tracking-tight text-foreground", className)} {...props} />;
}
...
export function AlertCancelButton(props: React.ComponentPropsWithoutRef<typeof AlertDialogPrimitive.Cancel>): JSX.Element {
  return (
    <AlertDialogPrimitive.Cancel asChild {...props}>
      <Button variant="outline" className="border text-foreground hover:bg-accent hover:text-foreground">
        Cancel
      </Button>

    ...
    <section className="grid gap-4 lg:grid-cols-2">
      <SettingsCard title={t("settings.appearance")}>
        ...
        <div className="mt-4 space-y-2">
          <label className="text-xs font-medium text-on-surface-variant dark:text-on-surface-variant">{t("settings.accentColor")}</label>
          <div className="flex gap-2">
            {(["zinc", "blue", "green", "rose", "amber", "violet"] as const).map((color) => (
              <button
                ...
                className={`h-8 w-8 rounded-full border-2 transition-all ${
                  settings.accentColor === color ? "border-primary scale-110" : "border-outline dark:border-outline"
                }`}
        ...
        <div className="mt-4">
          <label className="flex items-center gap-2 text-sm font-medium text-text-secondary dark:text-text-secondary">
            <input
              type="checkbox"
      ...
      <SettingsCard title={t("settings.language")}>
        <p className="mb-3 text-xs text-on-surface-variant dark:text-on-surface-variant">{t("settings.languageDescription")}</p>
        <div className="flex gap-2">
          {LANGUAGES.map((lang) => (
            <button
              key={lang.code}
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <input
              ...
              disabled={!settings.ttsEnabled}
              className="h-1.5 w-full cursor-pointer appearance-none rounded-full bg-surface-container-high accent-zinc-700 disabled:opacity-50 dark:bg-surface-container dark:accent-zinc-300"
            />
